...
import { addToCart,addToCartAct} from '../actions/cartActions';
...
const CartScreen = (props) => {
  ...
  const addFromCartHandlerM=(productId,qtyM)=>{
    let OrgQty=qtyM-1;
if(OrgQty<=0){
  ...
  OrgQty=1;
     ...
     dispatch( 
       addToCart(productId,OrgQty)
                )   
  }
  
  const addFromCartHandlerP=(productId,qtyM)=>{
    let OrgQty=qtyM+1;
     dispatch( 
       addToCart(productId,OrgQty)
                )   
  ...
  const mycountm=(pri,q)=>{
   ...
   // setQcount(qcount-1)   
    ...
    //  addToCart("5ffc315655c1e72bac4e1e13", 21)
    // )
   // alert("hjfghj")
   }
    ...
    `}</style>
      <section className="main_section cart">
        ...
        <div className="row">
          ...
          <h3 className="text-center"><i className="fa fa-shopping-cart"></i> Shopping Cart </h3>
     ...
     Hi Hanuman</div>))
        ...
        <div className="shopping-cart">
          <div className="column-labels">
            ...
            <label className="product-quantity text-center">Quantity</label>
        ...
        <div className="product" key={item.product}>
            ...
            {/* <input type="text" className="form-control" onChange={(e) => fdiscount(e.target.value)} id="discount" name="discount" placeholder="discount" /> */}
              ...
              {/* <button className="quantity-btn" prccoid={item.product} value={10}  */}
              
              
              {/* // onClick={(e) => mycountm(item.product,e.target.value)} */}

              <button className="quantity-btn"   onClick={(e) => addFromCartHandlerM(item.product,item.qty)}  data-quantity-minus><svg viewBox="0 0 409.6 409.6">
                  <g>
                    <g>
...
<input type="number"  id={item.product}  className="quantity-input"  value={item.qty} onChange={(e) =>
                        dispatch(
                          addToCart(item.product, Number(e.target.value))
                        )
                      } name="quantity" />
  <button    onClick={(e) => addFromCartHandlerP(item.product,item.qty)} className="quantity-btn" data-quantity-plus><svg viewBox="0 0 426.66667 426.66667">
      <path d="m405.332031 192h-170.664062v-170.667969c0-11.773437-9.558594-21.332031-21.335938-21.332031-11.773437 0-21.332031 9.558594-21.332031 21.332031v170.667969h-170.667969c-11.773437 0-21.332031 9.558594-21.332031 21.332031 0 11.777344 9.558594 21.335938 21.332031 21.335938h170.667969v170.664062c0 11.777344 9.558594 21.335938 21.332031 21.335938 11.777344 0 21.335938-9.558594 21.335938-21.335938v-170.664062h170.664062c11.777344 0 21.335938-9.558594 21.335938-21.335938 0-11.773437-9.558594-21.332031-21.335938-21.332031zm0 0" /></svg>
            ...
            <div className="product-line-price">{item.mrp* item.qty}</div>
          </div>
          ...
          <div className="totals-item">
              <label>Items</label>
              <div className="totals-values" id="cart-tax">{cartItems.reduce((a, c) => a + c.qty, 0)}</div>
            </div>
            <div className="totals-item">
              <label>Total</label>
              <div className="totals-value" id="cart-subtotal"> 
                {cartItems.reduce((a, c) => a + c.mrp * c.qty, 0)}</div>
            </div>
